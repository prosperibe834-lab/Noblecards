...
class AuthenticationService {
  ...
  static Future<void> initialize() async {
    ...
    _hasPersistedSession =
        (await _secureStorage!.read(key: _accessKey))?.isNotEmpty == true;
  }
  ...
  void _debugLog(
    ...
    print(
      '[AuthDebug] $method $path status=$statusCode tokenExists=$tokenExists expiry=$safeExpiry',
    );
  }
  ...
  Future<void> resendPasswordResetOtp({required String email}) async {
    await _request(
      'POST',
      '/auth/resend-otp',
      body: {'email': email.trim(), 'purpose': 'password-recovery'},
    );
  }
  ...
  Future<void> updatePassword(String newPassword) async {
    ...
    if (email == null || code == null)
      throw Exception('Your password reset session has expired.');
    await _request(
      'POST',
      '/auth/reset-password',
  ...
  Future<void> saveBiometricSession() async {
    ...
    if (accessToken == null || refreshToken == null) {
      throw Exception(
        'Your secure session is unavailable. Please log in again.',
      );
    }
  ...
  Future<Map<String, dynamic>> saveUserProfile({
    ...
    if (updatedUser == null)
      throw Exception('Profile update did not return the saved user.');
    final refreshedData = await _request(
      'GET',
      '/users/me',
      authenticated: true,
    );
  ...
  Future<Map<String, dynamic>> _request(
    ...
    _debugLog(
      ...
      tokenExists: accessToken != null && accessToken.isNotEmpty,
      expiry: accessToken == null ? null : _safeTokenExpiry(accessToken),
    );
  ...
  Future<AuthResponse> _storeSession(Map<String, dynamic> data) async {
    ...
    if (data['refreshToken'] != null)
      await _save(_refreshKey, data['refreshToken'] as String);
    _currentUser = user;
  ...
  String _friendlyMessage(Object message) {
    ...
    if (text.contains('wait') || text.contains('too many'))
